class @ChatClass
  ...
  sendMessage: (event) =>
    ...
    @dispatcher.trigger 'new_message', { body: msg_body , group_id: group_id, man_id: man_id, woman_id: woman_id, own_id: own_id, own_name: own_name}
    $('#msgbody').val('')

  receiveMessage: (message) =>
    # 受け取ったデータをappend
    $('#chat').append "#{message.own_name} : #{message.body}<br/>"
